...
/* ── Inline field ── */
function InlineField({ label, value, onChange, multiline = false, mono = false, hint }: {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between">
        <label className="text-[10px] font-bold tracking-[0.18em] uppercase" style={{ color: focused ? '#60a5fa' : 'rgba(255,255,255,0.3)', transition: 'color 0.15s' }}>
          {label}
        </label>
        {hint && <span className="text-[10px] text-white/20">{hint}</span>}
        <span className={`text-[10px] tabular-nums ${over ? 'text-red-400' : 'text-white/20'}`}>
...
function BulletEditor({ label, items, onChange, color = '#60a5fa' }: {
  ...
  return (
    <div className="flex flex-col gap-2">
      <label className="text-[10px] font-bold tracking-[0.18em] uppercase text-white/30">{label}</label>
      <div className="flex flex-col gap-1.5">
        {items.map((item, i) => (
          <div
            key={i}
            ...
            style={{
              background: dragOver === i ? 'rgba(96,165,250,0.06)' : 'rgba(255,255,255,0.02)',
              border: `1px solid ${dragOver === i ? 'rgba(96,165,250,0.25)' : 'rgba(255,255,255,0.06)'}`,
              cursor: 'grab',
            }}
          >
            {/* Drag handle */}
            ...
            <button type="button" onClick={() => remove(i)}
              className="shrink-0 w-6 h-6 rounded-md flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-150 hover:bg-red-500/10"
      ...
      <button type="button" onClick={add}
        className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold w-fit transition-all duration-150"
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.07)', background: '#ffffff' }}>
      ...
      <div className="flex items-center gap-2 px-4 py-2.5" style={{ background: 'rgba(255,255,255,0.04)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        ...
        <span className="text-[10px] font-bold tracking-[0.18em] uppercase text-white/30">Live Preview</span>
        <span className="ml-auto text-[10px] text-white/20">/promotions/welcome-bonus</span>
      </div>

      <div className="p-6 grid grid-cols-1 gap-6" style={{ background: '#fff' }}>
        {/* Left */}
        <div className="flex flex-col gap-3">
          <h2 className="text-xl font-black leading-tight" style={{ color: '#0f1e3c', fontFamily: 'var(--font-inter)' }}>
        ...
        {/* Right card */}
        <div className="rounded-2xl p-5 flex flex-col gap-3 relative overflow-hidden" style={{ background: 'linear-gradient(135deg,#0f1e3c 0%,#1535a0 60%,#0d1b5e 100%)' }}>
          <p className="text-xs font-bold tracking-widest uppercase" style={{ color: '#93c5fd' }}>{content.cardLabel}</p>
          <p className="text-3xl font-black text-white">{content.cardPercent}</p>
...
function StatusChip({ status }: { status: 'idle' | 'saving' | 'saved' | 'error' }) {
  ...
  const map = {
    saving: { color: '#fbbf24', bg: 'rgba(251,191,36,0.1)', border: 'rgba(251,191,36,0.2)', label: 'Saving…', icon: (
      <svg className="animate-spin" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <circle cx="12" cy="12" r="10" strokeOpacity="0.3"/><path d="M12 2a10 10 0 0 1 10 10"/>
      </svg>
    )},
    saved: { color: '#4ade80', bg: 'rgba(74,222,128,0.08)', border: 'rgba(74,222,128,0.2)', label: 'Saved', icon: (
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
    )},
    error: { color: '#f87171', bg: 'rgba(248,113,113,0.08)', border: 'rgba(248,113,113,0.2)', label: 'Error', icon: (
...
function Section({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.07)' }}>
      <div className="flex items-center gap-2.5 px-5 py-4" style={{ background: 'rgba(255,255,255,0.03)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <span className="text-white/40">{icon}</span>
        <span className="text-xs font-bold tracking-[0.15em] uppercase text-white/40">{title}</span>
      </div>
      <div className="p-5 flex flex-col gap-5 group" style={{ background: 'rgba(255,255,255,0.015)' }}>
        {children}
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen" style={{ background: '#070d1a' }}>
      ...
      <div className="sticky top-0 z-30 flex items-center justify-between px-8 py-4"
        style={{ background: 'rgba(7,13,26,0.85)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="flex items-center gap-3">
          <div>
            ...
            <h1 className="text-lg font-black text-white tracking-tight">Welcome Bonus Editor</h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex xl:hidden rounded-xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <a href="/promotions/welcome-bonus" target="_blank"
      ...
      {loading ? (
        ...
        <div className="p-6 xl:p-8">
          {/* Desktop: split panels | Mobile: tabs */}
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <div className={`flex flex-col gap-4 ${tab === 'preview' ? 'hidden xl:flex' : 'flex'}`}>
              ...
              <p className="text-[11px] text-white/20 text-center pb-2">
                ✦ Changes are auto-saved 0.9s after you stop typing
              </p>
            </div>
      ...
      <style>{`
        ...
        textarea { transition: border-color 0.15s, background 0.15s; }
        input[type="text"] { transition: border-color 0.15s, background 0.15s; }
      `}</style>
    </div>
  )
}
