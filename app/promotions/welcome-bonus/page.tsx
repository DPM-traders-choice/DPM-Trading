...
export default function WelcomeBonusPage() {
  const [content, setContent] = useState<BonusContent>(DEFAULT)
  ...
  return (
    <main className="overflow-hidden" style={{ background: '#ffffff' }}>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div
              ...
              style={{ background: '#ffffff', border: '1px solid #e2e8f0', ['--d' as string]: `${i * 100}ms` }}
            >
      ...
      {/* ── Body Content (editable from admin) ── */}
      <section className="py-24 md:py-32 px-6 md:px-12 bg-white">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="wb-reveal flex flex-col gap-6" style={{ ['--d' as string]: '0ms' }}>
            ...
            <p className="text-lg md:text-xl font-semibold text-[#0f1e3c]">{content.subheading}</p>
            <p className="text-base md:text-lg text-gray-500 leading-relaxed">{content.bodyText}</p>
            <ul className="flex flex-col gap-3">
              {content.bullets.filter(Boolean).map((item, i) => (
          ...
          <div
            ...
            style={{ background: 'linear-gradient(135deg, #0f1e3c 0%, #1535a0 60%, #0d1b5e 100%)', ['--d' as string]: '150ms' }}
          >
            <div className="absolute bottom-0 right-0 w-44 md:w-52 pointer-events-none select-none">
              <Image src="/copyTrading/card-2-428w.webp" alt="" width={428} height={428} className="w-full h-auto object-contain" />
            </div>
            <p className="text-blue-300 text-sm font-bold tracking-[0.15em] uppercase">{content.cardLabel}</p>
            <p className="text-white text-4xl font-black">{content.cardPercent}</p>
            <p className="text-white/70 text-base leading-relaxed">{content.cardDesc}</p>
      ...
      <section className="py-24 md:py-32 px-6 md:px-12" style={{ background: '#F0F4F9' }}>
        <div className="max-w-7xl mx-auto flex flex-col items-center gap-14">
          <div className="wb-reveal flex flex-col items-center text-center gap-4 max-w-2xl" style={{ ['--d' as string]: '0ms' }}>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-black leading-[1.1] tracking-tight text-[#0f1e3c]" style={{ fontFamily: 'var(--font-inter)' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
            {[
              ...
              <div key={step.n} className="wb-reveal flex flex-col gap-4 rounded-2xl bg-white p-8" style={{ border: '1px solid #e2e8f0', ['--d' as string]: `${i * 100}ms` }}>
                <span className="text-5xl font-black leading-none" style={{ background: 'linear-gradient(180deg, #2563eb 0%, #93c5fd 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                  {step.n}
                </span>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="wb-reveal flex flex-col gap-6" style={{ ['--d' as string]: '150ms' }}>
            <p className="text-base md:text-lg text-gray-500 leading-relaxed">
              Unlock extra trading power with DPM by taking advantage of our 20% Welcome Bonus. This promotion is available to both new and existing clients with specific Welcome bonus trading accounts in any supported currency, allowing you to enhance your margin and trading potential from the start.
            </p>
            <p className="text-base md:text-lg text-gray-500 leading-relaxed">
              Clients can receive up to $500 in bonus credit with a minimum deposit of $100. The bonus applies to all DPM trading instruments. Terms and Conditions apply please review them carefully to ensure eligibility before participating in this offer.
            </p>
            <div>
              <a href="/about-us/legal-documents" className="inline-flex items-center justify-center rounded-lg border-2 border-[#0f1e3c] text-[#0f1e3c] hover:bg-[#0f1e3c] hover:text-white transition-colors duration-300 px-8 py-3.5 text-base font-semibold">
                Learn More
              </a>
            </div>
          </div>
      ...
      <style>{`
        ...
        .wb-reveal.wb-visible { opacity: 1; transform: translateY(0); }
